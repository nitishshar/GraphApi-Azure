...
namespace B2CGraphShell
{
    public class Program
    {
        ...
        private static List<Role> appRoles { get; set; } = new List<Role>();
        ...
        private static void GetAppRoles()
        {
            string result;
            result = client.GetAppRoles().Result;
            dynamic res = JsonConvert.DeserializeObject(result);
            var applicationRoles = (res.value[0] as JObject)["appRoles"];
            foreach (var item in applicationRoles)
            {
                appRoles.Add(new Role { RoleId = item["id"].ToString(), RoleDescription = item["description"].ToString(), RoleName = item["displayName"].ToString() });
            }
            Console.ForegroundColor = ConsoleColor.White;
            Console.WriteLine(JsonConvert.SerializeObject(appRoles, Formatting.Indented));
        }
